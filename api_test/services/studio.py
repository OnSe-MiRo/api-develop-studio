...
from api_test.request_profiles import normalize_profiles, resolve_auth_profile, profiles_for_client
from api_test.authorization import AuthorizationError, apply_authorization, authorization_sensitive_values, sensitive_value_variants
...
from api_test.runner import execute_http_call, format_network_error, project_request_settings, resolve_request_url, encode_multipart_form_data
from api_test.project_variables import resolve_project_references
...
def execution_metadata(body: dict[str, object]) -> tuple[list[str], list[dict[str, object]]]:
    ...
    def add(kind: str, reference: str, document: object = None, preview: bool = False) -> None:
        if document is None:
            try:
    ...
    if body.get("inlineCase") is not None:
        ...
        add("case", reference if isinstance(reference, str) else "preview/unsaved/unsaved_case.json", body["inlineCase"], True)
        ...
        for reference in body.get("cases", []):
            if isinstance(reference, str):
                add("case", reference)
        pipelines = [item for item in body.get("pipelines", []) if isinstance(item, str)]
        if not pipelines and not body.get("cases"):
            pipelines = project_json_files(PIPELINE_ROOT, None)
            if not example_project_enabled():
                example_pipelines = set(project_json_files(PIPELINE_ROOT, EXAMPLE_PROJECT_REFERENCE))
                pipelines = [reference for reference in pipelines if reference not in example_pipelines]
...
def normalize_project_document(
    ...
    try:
        if "variables" in document:
            document["variables"] = normalize_project_variables(document["variables"], (existing or {}).get("variables"))
        elif existing is not None:
            document["variables"] = existing.get("variables", {})
        return normalize_profiles(document, existing)
...
def handle_api_request(body: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(body.get("url"), str) or not body["url"].strip():
        raise ApiError("URL을 입력하세요.")
    resolved_secrets = set()
    if not body.get("project") and (body.get("environment") or body.get("auth_profile")):
        raise ApiError("환경 또는 인증 프로필을 사용하려면 프로젝트를 선택하세요.")
    if body.get("project"):
        try:
            settings = project_request_settings({"project": body["project"]}, PROJECT_ROOT, body.get("environment"))
            body = resolve_auth_profile(body, settings.auth_profiles)
            body, resolved_secrets = resolve_project_references(body, settings.variables, settings.encrypted_variables)
    ...
    sensitive_values: set[str] = set(resolved_secrets)
    ...
    if req_body is not None and (req_body != "" or body.get("body_format") == "json"):
        if isinstance(req_body, (dict, list)) or body.get("body_format") == "json":
    ...
    if "text" in body:
        ...
        headers_dict.setdefault("Content-Type", "text/plain")
    if "form_data" in body:
        try:
            data, content_type = encode_multipart_form_data(body["form_data"], CASE_ROOT)
    ...
    sensitive_values.update(value for key, value in headers_dict.items() if key.lower() == "authorization")
    sensitive_values = sensitive_value_variants(sensitive_values)
    ...
    size_bytes = len(raw_response_body.encode("utf-8"))
    try:
    ...
    def mask_payload(value):
        ...
        return mask_sensitive_values(value, sensitive_values) if isinstance(value, str) else value

    parsed_body = mask_payload(parsed_body)
    raw_secrets = sensitive_values | {json.dumps(value)[1:-1] for value in sensitive_values}
    raw_response_body = mask_sensitive_values(raw_response_body, raw_secrets)
