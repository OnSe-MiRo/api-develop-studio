...
> 보관 문서: 구현 중 발견한 결함과 수정 이력이다. 현재 사용법과 상태는 [`../../mock-server.md`](../../mock-server.md) 및 [`../../api-development-progress.md`](../../api-development-progress.md)를 따른다.

## 개발 반영 — 검증 대기 (2026-09-21)

사용자 요청에 따라 아래 2차 리뷰의 잔여 6개 항목을 개발에 반영했다. 동일 설정의 state 보존, 해석된 operation 목록의 UI 연결, schema 제약 확인/지원 한계 명시 오류, override 타입 검사, 명시적 example/media 선택에 대한 CRUD 비변경 응답 재생, smoke deadline 제출/취소 로직을 수정했다. 회귀 테스트를 준비했지만 이번에는 실행하지 않았다. 후속 검증에서 기존 리뷰 재현, 신규 테스트, 명세 소스 3종의 실제 UI, HTTP·종료/동시성 경계를 확인해야 한다. 아래 2차 리뷰 결과는 수정 전 이력이며, 지금 수정된 코드가 통과했다는 판정은 아니다.
...
원래 [개발 인계 프롬프트](development-prompt.md)와 이 리뷰의 R1–R8 및 경계 항목을 반영하라. 먼저 각 재현을 실패하는 회귀 테스트로 만들고 수정하라. 기존 사용자 변경을 보존하고 범위 밖 리팩터링은 하지 않는다. 구현 후 전체 테스트·생성 검사·build·실제 HTTP/브라우저·제한된 smoke를 다시 수행해 두 진행 문서를 동기화한다. 미수행 검증과 미해결 문제를 남김없이 보고하고, 독립 재검증 전에는 MOCK-1 전체 완료로 표시하지 않는다. 커밋·병합·푸시는 하지 않는다.
