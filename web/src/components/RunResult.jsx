...
function RunResult({ result, onCancel }) {
  ...
  const statusLabels = { reconnecting: '상태 확인 중', queued: '실행 대기 중', running: '실행 중', cancelling: '취소 중', cancelled: '실행 취소됨', timeout: '실행 시간 초과', passed: '실행 완료', failed: '실행 실패', error: '실행 오류' }
  ...
  return <section className={`run-result ${active ? 'pending' : result.error || result.exitCode || ['timeout', 'cancelled', 'error'].includes(result.status) ? 'failure' : 'success'}`}>
    ...
    {result.result?.targets?.length > 0 && <div><strong>단계별 결과</strong><ul>{result.result.targets.map((item, index) => <li key={index}>{item.caseId} · {statusLabels[item.status] || item.status}{item.httpStatus != null ? ` · HTTP ${item.httpStatus}` : ''}{item.errorCategory ? ` · ${item.errorCategory}` : ''}</li>)}</ul></div>}
