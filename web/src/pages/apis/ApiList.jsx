...
function SwaggerOperation({ operation, onEdit }) {
...
function ApiList({ projects, projectRef, project, onProjectChange, onNavigate, onProjectList, onCreate, refresh }) {
  ...
  }, [projectRef, project?.docs_url, project?._storage?.revision, reload])
  return <div className="workspace"><AuthorSidebar active="apis" projects={projects} projectRef={projectRef} project={project} onProjectChange={onProjectChange} onNavigate={onNavigate} onProjectList={onProjectList} /><main className="editor"><section className="card"><div className="section-header"><div><p className="eyebrow">OPENAPI AUTHORING</p><h2>API 목록 <span className="count">{operations.length}</span></h2></div>{projectRef === 'example-api.json' ? <span className="hint">읽기 전용 예제</span> : <button className="primary" disabled={!projectRef} onClick={onCreate}>＋ API 작성</button>}</div>{operations.length ? <div className="swagger-list">{groupDocOperationsByTag(operations).map(([tag, items]) => <section className="swagger-tag-group" key={tag}><h3>{tag} <span>{items.length}</span></h3>{items.map(operation => <SwaggerOperation key={operation.id} operation={operation} onEdit={projectRef === 'example-api.json' ? null : setEditing} />)}</section>)}</div> : <div className="empty">표시할 API가 없습니다.</div>}</section><MockServerPanel key={`mock-${projectRef}`} projectRef={projectRef} project={mockProject} /><CoveragePanel key={`${projectRef}:${project?._storage?.revision}`} projectRef={projectRef} revision={project?._storage?.revision} /><ContractPanel projectRef={projectRef} project={project} />{editing && <OperationEditor key={`${projectRef}-${editing.id}`} operation={editing} projectRef={projectRef} project={project} onCancel={() => setEditing(null)} onSaved={async () => { setEditing(null); await refresh(projectRef); setReload(current => current + 1) }} />}{notice && <p className="notice" role="status">{notice}</p>}</main></div>
}
