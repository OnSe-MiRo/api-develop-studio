...
function ProjectList({ projects, projectDetails, activeProject, onOpenProject, onCreateProject, onEditProject, refresh }) {
  ...
  return <main className="project-page"><section className="card project-list-card"><div className="section-header"><div><p className="eyebrow">SELECT PROJECT</p><h2>프로젝트 목록</h2></div><button className="primary" onClick={onCreateProject}>＋ 새 프로젝트 만들기</button></div>{projects.length ? <div className="project-grid">{projects.map(reference => { const isExample = reference === 'example-api.json'; return <article className={`project-card ${activeProject === reference ? 'active' : ''}`} key={reference}><button className="project-card-open" onClick={() => onOpenProject(reference)}><span className="project-card-label">{isExample ? 'READ ONLY EXAMPLE' : 'PROJECT'}</span><strong>{reference.replace(/\.json$/, '')}</strong><small>{projectDetails[reference]?.base_url || ''}</small><span className="project-card-action">API 테스트 열기 <b>→</b></span></button><div className="project-card-actions"><button className="project-card-edit" aria-label={`${reference} 프로젝트 ${isExample ? '보기' : '수정'}`} title={isExample ? '예제 값 보기' : '프로젝트 수정'} onClick={() => onEditProject(reference)}>{isExample ? '보기' : '수정'}</button>{!isExample && <button className="project-card-delete" aria-label={`${reference} 프로젝트 삭제`} title="프로젝트 삭제" onClick={() => removeProject(reference)}>×</button>}</div></article> })}</div> : <div className="empty">등록된 프로젝트가 없습니다. 새 프로젝트를 만들어 시작하세요.</div>}</section>{notice && <p className="notice">{notice}</p>}</main>
}
