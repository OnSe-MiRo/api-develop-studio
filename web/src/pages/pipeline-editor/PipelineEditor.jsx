...
function PipelineEditor({ caseItems, refresh, projectRef, project, onNavigate, onProjectList, pipelineReference, onBack }) {
  ...
  const [generator, setGenerator] = useState({ name: '', type: 'uuid', min: 0, max: 100 })
  const [draft, setDraft] = useState({ name: '', case: '', retry: '', interval: '', continue: false })
  const [selected, setSelected] = useState('')
  const [notice, setNotice] = useState('')
  const { result, setResult, busy, start: startRun, cancel: cancelRun } = useRunJob(`${projectRef}:pipeline:${pipelineReference || "new"}`)
  ...
  useEffect(() => {
    ...
    else { setFileName('new_pipeline.json'); setDefaults({ retry: 0, retry_interval_seconds: 0 }); setSteps([]); setGenerators({}); setSeed(''); setDraft({ name: '', case: '', retry: '', interval: '', continue: false }); setMappingEditor(null); setSelected(''); setStorageMeta(null); setNotice('새 파이프라인을 작성하세요.') }
  }, [pipelineReference, projectRef])
  const load = async reference => {
    if (!reference) return
    try { const data = await api(`/api/pipelines/${encodeURIComponent(reference)}`); setFileName(reference); setDefaults(data.defaults || { retry: 0, retry_interval_seconds: 0 }); setSteps(data.steps || []); setGenerators(data.generators || {}); setSeed(data.seed ?? ''); setMappingEditor(null); setSelected(reference); setStorageMeta(data._storage || null); setNotice(`불러옴: ${reference}`) } catch (error) { setNotice(error.message) }
  }
  const addStep = () => {
    const selectedCase = draft.case || caseItems[0]
    if (!draft.name || !selectedCase) return setNotice('단계 이름과 케이스를 선택하세요.')
    if (steps.some(step => step.name === draft.name)) return setNotice('단계 이름은 고유해야 합니다.')
    const step = { name: draft.name, case: selectedCase, phase: draft.phase || 'test' }
    if (draft.extractName) {
      if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(draft.extractName) || !/^(body(?:\.[\w-]+)*|status)$/.test(draft.extractPath || '')) return setNotice('추출 변수 이름과 응답 경로를 확인하세요.')
      step.extract = { [draft.extractName]: draft.extractPath }
    }
    ...
    setSteps(current => [...current, step].sort((a, b) => ['setup', 'test', 'teardown'].indexOf(a.phase || 'test') - ['setup', 'test', 'teardown'].indexOf(b.phase || 'test'))); setDraft(current => ({ ...current, name: '', retry: '', interval: '', continue: false, extractName: '', extractPath: '' }))
  }
  const pipelineDocument = () => {
    if (!projectRef) throw new Error('프로젝트를 먼저 선택하세요.')
    if (!steps.length) throw new Error('최소 한 개의 단계를 추가하세요.')
    return { project: projectRef, generators, ...(seed === '' ? {} : { seed }), defaults: { retry: Number(defaults.retry), retry_interval_seconds: Number(defaults.retry_interval_seconds) }, steps }
  }
  const save = async () => {
  ...
  const runOnly = async () => {
    try {
      ...
      await startRun({ environment, inlinePipeline: pipelineDocument() })
      setNotice('저장하지 않고 실행 작업을 제출했습니다.')
    } catch (error) { setResult({ error: error.message }); setNotice(error.message) }
  }
  const run = async () => { if (!(await save())) return; try { await startRun({ environment, pipelines: [ref] }) } catch (error) { setResult({ error: error.message }) } }
  ...
  return <div className="workspace"><TestSidebar active="pipeline" projectRef={projectRef} project={project} onNavigate={onNavigate} onProjectList={onProjectList} /><main className="editor"><ExecutionEnvironment project={project} value={environment} onChange={setEnvironment} /><section className="card"><div className="section-header"><div><p className="eyebrow">PIPELINE SETTINGS</p><h2>{selected ? '파이프라인 설정' : '새 파이프라인'}</h2></div><div className="actions"><button className="ghost" onClick={onBack}>목록으로</button>{selected && <button className="danger-button" onClick={removePipeline}>삭제</button>}<button className="ghost" onClick={save}>저장</button><button className="ghost" disabled={busy} onClick={runOnly}>실행만</button><button className="primary" disabled={busy} onClick={run}>저장 후 실행</button></div></div><div className="form-grid three"><Field label="파일명"><input value={fileName} onChange={event => setFileName(event.target.value)} /></Field><Field label="기본 재시도"><input type="number" min="0" value={defaults.retry} onChange={event => setDefaults(current => ({ ...current, retry: event.target.value }))} /></Field><Field label="기본 간격 (초)"><input type="number" min="0" step="0.1" value={defaults.retry_interval_seconds} onChange={event => setDefaults(current => ({ ...current, retry_interval_seconds: event.target.value }))} /></Field></div></section><section className="card"><h2>실행별 테스트 데이터</h2><p className="hint">요청에서 {'${run.변수명}'}으로 사용합니다. 응답 추출 변수도 동일하게 사용할 수 있습니다. 준비 → 테스트 → 정리 순서로 배치하세요. 정리 단계는 실패 후에도 실행됩니다.</p><Field label="고정 seed (비워두면 매 실행 새 값)"><input value={seed} onChange={event => setSeed(event.target.value)} /></Field><div className="form-grid"><Field label="변수 이름"><input value={generator.name} onChange={event => setGenerator(current => ({ ...current, name: event.target.value }))} /></Field><Field label="생성 방식"><select value={generator.type} onChange={event => setGenerator(current => ({ ...current, type: event.target.value }))}><option value="uuid">UUID</option><option value="timestamp">UTC 시각</option><option value="integer">정수 범위</option></select></Field>{generator.type === 'integer' && <><Field label="최솟값"><input type="number" value={generator.min} onChange={event => setGenerator(current => ({ ...current, min: event.target.value }))} /></Field><Field label="최댓값"><input type="number" value={generator.max} onChange={event => setGenerator(current => ({ ...current, max: event.target.value }))} /></Field></>}</div><button className="ghost" onClick={() => { if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(generator.name)) return setNotice('영문 변수 이름을 입력하세요.'); setGenerators(current => ({ ...current, [generator.name]: { type: generator.type, ...(generator.type === 'integer' ? { min: Number(generator.min), max: Number(generator.max) } : {}) } })); setGenerator(current => ({ ...current, name: '' })) }}>변수 추가</button>{Object.entries(generators).map(([name, definition]) => <p key={name}>{name} · {definition.type}{definition.type === 'integer' ? ` (${definition.min} ~ ${definition.max})` : ''} <button className="ghost" onClick={() => setGenerators(current => Object.fromEntries(Object.entries(current).filter(([key]) => key !== name)))}>삭제</button></p>)}</section><section className="card"><div className="section-header"><div><p className="eyebrow">ADD STEP</p><h2>테스트 단계 추가</h2></div></div><div className="form-grid step-grid"><Field label="케이스" wide><select value={draft.case} onChange={event => setDraft(current => ({ ...current, case: event.target.value }))} disabled={!projectRef}>{caseItems.map(item => <option key={item}>{item}</option>)}</select></Field><Field label="단계 종류"><select value={draft.phase || 'test'} onChange={event => setDraft(current => ({ ...current, phase: event.target.value }))}><option value="setup">준비 (setup)</option><option value="test">테스트</option><option value="teardown">정리 (teardown)</option></select></Field><Field label="응답 추출 변수 이름"><input value={draft.extractName || ''} onChange={event => setDraft(current => ({ ...current, extractName: event.target.value }))} placeholder="created_id" /></Field><Field label="추출 응답 경로"><input value={draft.extractPath || ''} onChange={event => setDraft(current => ({ ...current, extractPath: event.target.value }))} placeholder="body.id" /></Field><Field label="단계 이름"><input value={draft.name} onChange={event => setDraft(current => ({ ...current, name: event.target.value }))} placeholder="get_user" /></Field><Field label="재시도 (선택)"><input type="number" min="0" value={draft.retry} onChange={event => setDraft(current => ({ ...current, retry: event.target.value }))} /></Field><Field label="간격 (선택)"><input type="number" min="0" step="0.1" value={draft.interval} onChange={event => setDraft(current => ({ ...current, interval: event.target.value }))} /></Field></div><div className="step-actions"><label className="toggle"><input type="checkbox" checked={Boolean(draft.external)} onChange={event => setDraft(current => ({ ...current, external: event.target.checked, retry: event.target.checked ? '0' : '', continue: false }))} /><span>Setup · 승인된 외부 API 1회 호출</span></label><label className="toggle"><input type="checkbox" checked={draft.continue} onChange={event => setDraft(current => ({ ...current, continue: event.target.checked }))} /><span>실패해도 다음 단계 실행</span></label><button className="primary" onClick={addStep}>＋ 단계 추가</button></div></section><section className="card"><div className="section-header"><div><p className="eyebrow">EXECUTION ORDER</p><h2>실행 순서 <span className="count">{steps.length}</span></h2></div></div><div className="steps">{steps.length ? steps.map((step, index) => <div className="step" key={step.name}><span className="order">{String(index + 1).padStart(2, '0')}</span><div><strong>{step.external_once ? 'Setup · 외부 1회 · ' : `${step.phase || 'test'} · `}{step.name}</strong><small>{step.case}{step.extract ? ` · 추출: ${Object.keys(step.extract).join(', ')}` : ''}{step.input_mappings?.length ? ` · 값 전달 ${step.input_mappings.length}개` : ''}</small></div><div className="step-meta">재시도 {step.retry ?? '기본값'} · 간격 {step.retry_interval_seconds ?? '기본값'}</div><div className="row-actions"><button className="mapping-button" onClick={() => openMappings(index)}>값 전달</button><button className="icon" onClick={() => move(index, -1)}>↑</button><button className="icon" onClick={() => move(index, 1)}>↓</button><button className="icon danger" onClick={() => setSteps(current => current.filter((_, itemIndex) => itemIndex !== index))}>×</button></div></div>) : <div className="empty">API 케이스를 먼저 저장한 뒤 단계로 추가하세요.</div>}</div></section>{mappingEditor && <ValueMappingEditor editor={mappingEditor} steps={steps} onUpdateDraft={updateMappingDraft} onAdd={addMapping} onRemove={index => setMappingEditor(current => current ? { ...current, mappings: current.mappings.filter((_, itemIndex) => itemIndex !== index) } : current)} onSave={saveMappings} onCancel={() => setMappingEditor(null)} />}{notice && <p className="notice">{notice}</p>}<RunResult result={result} onCancel={cancelRun} onClose={() => setResult(null)} /></main></div>
}
