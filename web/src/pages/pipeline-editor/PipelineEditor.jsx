...
function PipelineEditor({ caseItems, refresh, projectRef, project, onNavigate, onProjectList, pipelineReference, onBack }) {
  ...
  const runOnly = async () => {
    try {
      ...
      setResult(await api('/api/run', { method: 'POST', body: JSON.stringify({ environment, inlinePipeline: pipelineDocument() }) }))
      setNotice('저장하지 않고 실행했습니다.')
    } catch (error) { setResult({ error: error.message }); setNotice(error.message) }
  }
  const run = async () => { if (!(await save())) return; try { setResult(await api('/api/run', { method: 'POST', body: JSON.stringify({ environment, pipelines: [ref] }) })) } catch (error) { setResult({ error: error.message }) } }
