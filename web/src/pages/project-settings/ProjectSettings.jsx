...
const emptyProjectForm = () => ({
  ...
  baseUrls: [], plainVariables: [], secretVariables: [], environments: '{}', authProfiles: '{}', defaultEnvironment: '',
})
...
function ExampleProjectSettingsView({ form, onCancel }) {
  const documentSource = form.docsUrl || form.docsFile?.name || form.docsBundle?.entrypoint || '없음'
  return <main className="project-page">
    <section className="card">
      <div className="section-header"><div><p className="eyebrow">READ ONLY EXAMPLE</p><h2>프로젝트 설정</h2></div><button className="ghost" onClick={onCancel}>목록으로</button></div>
      <p className="notice">내장 Example 프로젝트는 값을 확인하고 실행할 수 있으며 변경할 수 없습니다.</p>
      <div className="form-grid project-settings-grid"><Field label="프로젝트 이름"><input readOnly value={form.name} /></Field><Field label="기본 Base URL"><input readOnly value={form.baseUrl} /></Field></div>
      <section className="base-url-settings"><div className="proxy-settings-heading"><strong>추가 Base URL</strong></div>{form.baseUrls.length ? form.baseUrls.map(item => <div className="base-url-row" key={item.name}><input readOnly value={item.name} /><input readOnly value={item.url} /></div>) : <div className="project-variable-empty">추가 Base URL이 없습니다.</div>}</section>
      <section className="advanced-settings"><div className="advanced-content">
        <section className="proxy-settings"><div className="proxy-settings-heading"><strong>OpenAPI 설정</strong></div><Field label="문서 원본"><input readOnly value={documentSource} /></Field></section>
        <section className="project-variable-settings"><div className="proxy-settings-heading"><strong>프로젝트 공통 변수</strong></div>
          <div className="project-variable-list"><div className="project-variable-header"><span>구분</span><span>변수명</span><span>값</span></div>
            {form.plainVariables.map(item => <div className="project-variable-row" key={`plain-${item.name}`}><span>일반</span><code>{item.name}</code><code>{item.value}</code></div>)}
            {form.secretVariables.map(item => <div className="project-variable-row" key={`secret-${item.name}`}><span>예제 공개값</span><code>{item.name}</code><code>{item.value}</code></div>)}
          </div>
        </section>
        <section className="proxy-settings"><div className="proxy-settings-heading"><strong>Proxy·TLS 설정</strong></div><div className="form-grid project-settings-grid"><Field label="Proxy 사용"><input readOnly value={form.useProxy ? '사용' : '사용 안 함'} /></Field><Field label="HTTP Proxy URL"><input readOnly value={form.httpProxy || '없음'} /></Field><Field label="HTTPS Proxy URL"><input readOnly value={form.httpsProxy || '없음'} /></Field><Field label="TLS 인증서 검증"><input readOnly value={form.verify ? '사용' : '사용 안 함'} /></Field></div></section>
      </div></section>
    </section>
  </main>
}
...
function ProjectSettings({ projects, projectReference, onSaved, onCancel }) {
  ...
  const isExample = projectReference === 'example-api.json'
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const secretVariables = Object.entries(variables.secret || {}).map(([name, definition]) => ({ name, value: asText(definition?.value), configured: Boolean(definition?.configured) }))
        ...
        setForm({ environments: JSON.stringify(data.environments || {}, null, 2), authProfiles: JSON.stringify(data.auth_profiles || {}, null, 2), defaultEnvironment: data.default_environment || '', name: asText(data.name), baseUrl: asText(data.base_url), baseUrls, docsUrl: asText(data.docs_url), docsFile, docsBundle, useProxy: advanced.use_proxy !== false, sameProxy: Boolean(httpProxy && httpProxy === httpsProxy), httpProxy, httpsProxy, verify: advanced.verify ?? true, plainVariables, secretVariables })
  ...
  }, [form.docsUrl, form.useProxy, isExample])
  const save = async () => {
    try {
      ...
      payload.environments = JSON.parse(form.environments)
      for (const env of Object.values(payload.environments)) {
        for (const [name, definition] of Object.entries(env.variables?.secret || {})) {
          if (definition.configured && !definition.value) env.variables.secret[name] = { preserve: true }
        }
      }
      payload.auth_profiles = JSON.parse(form.authProfiles)
      payload.default_environment = form.defaultEnvironment
