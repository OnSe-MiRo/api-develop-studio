  ...
  return <div className="workspace">
    ...
    <main className="editor">
      <section className="card"><div className="section-header"><div><p className="eyebrow">CASE SETTINGS</p><h2>{selected ? 'API 케이스 설정' : '새 API 케이스'}</h2></div><div className="actions"><button className="ghost" onClick={onBack}>목록으로</button>{selected && <button className="danger-button" onClick={removeCase}>삭제</button>}<button className="ghost" onClick={save}>저장</button></div></div>
        ...
        <div className="docs-import">{docsSource ? <><div className="docs-source"><span>프로젝트 API 문서</span><code>{docsSource.label}</code></div><button className="ghost" onClick={() => loadDocs()}>문서 새로 불러오기</button>{docOperations.length > 0 && <Field label="문서 API 선택" wide><select value="" onChange={event => applyDocOperation(event.target.value)}><option value="">API를 선택하세요. ({docOperations.length}개)</option>{docOperations.map(operation => <option key={operation.id} value={operation.id}>{operation.method} {operation.path}{operation.summary ? ` — ${operation.summary}` : ''}</option>)}</select></Field>}</> : <p className="hint">프로젝트 설정에서 OpenAPI / Swagger 문서 URL 또는 JSON 파일을 등록하면 API 목록을 자동으로 불러옵니다.</p>}<p className="hint">API를 선택하면 Params·Headers·Body와 기대 응답 예시가 자동 입력됩니다.</p></div>
      </section>
      {availableProjectVariables.length > 0 && <section className="card project-variable-reference"><div><p className="eyebrow">PROJECT VARIABLES</p><h2>프로젝트 공통 변수</h2><p className="hint">URL·Params·Authorization·Headers·Body에서 참조식을 사용할 수 있습니다. 보안 변수의 실제 값은 화면에 표시되지 않습니다.</p></div><div className="project-variable-chips">{availableProjectVariables.map(variable => <button className={variable.secret ? 'secret' : ''} key={variable.name} onClick={() => copyProjectVariable(variable.name)}><span>{variable.secret ? '보안' : '일반'}</span><code>{`{{project.${variable.name}}}`}</code><small>복사</small></button>)}</div></section>}
      <section className="card project-variable-reference"><div className="section-header"><div><p className="eyebrow">CASE SECRET VARIABLES</p><h2>케이스 전용 보안 변수</h2><p className="hint">이 케이스에서만 사용할 API Key·토큰을 암호화해 저장합니다. URL·Params·Authorization·Headers·Body에서 <code>{'{{case.변수명}}'}</code>으로 참조하세요.</p></div></div><ProjectVariableRows secret items={form.secretVariables} onAdd={addCaseSecretVariable} onUpdate={updateCaseSecretVariable} onRemove={removeCaseSecretVariable} />{availableCaseVariables.length > 0 && <div className="project-variable-chips">{availableCaseVariables.map(variable => <button className="secret" key={variable.name} onClick={() => copyCaseVariable(variable.name)}><span>보안</span><code>{`{{case.${variable.name}}}`}</code><small>복사</small></button>)}</div>}</section>
      ...
      <section className="card"><div className="section-header"><div><p className="eyebrow">ASSERTION</p><h2>기대 응답</h2></div></div><div className="validation-method-section"><strong>검증 방법</strong><p className="hint">두 방법 중 하나만 선택하거나 둘 다 선택할 수 있습니다. Expected status는 선택과 관계없이 항상 검증합니다.</p><div className="validation-methods"><label className={`validation-method ${form.validateExact ? 'selected' : ''}`}><input type="checkbox" checked={form.validateExact} onChange={event => set('validateExact', event.target.checked)} /><span><strong>기대 응답 일치</strong><small>Expected body와 실제 응답의 값·구조를 비교합니다.</small></span></label><label className={`validation-method ${form.validateConditions ? 'selected' : ''}`}><input type="checkbox" checked={form.validateConditions} onChange={event => set('validateConditions', event.target.checked)} /><span><strong>변수별 조건</strong><small>선택한 변수의 범위·타입·존재·길이를 검사합니다.</small></span></label></div></div><div className="expected-controls"><Field label="Expected status"><input value={form.expectedStatus} onChange={event => set('expectedStatus', event.target.value)} /></Field><label className={`toggle ${form.validateExact ? '' : 'disabled'}`}><input type="checkbox" checked={form.strict} disabled={!form.validateExact} onChange={event => set('strict', event.target.checked)} /><span>strict 비교</span></label></div><div className="expected-body-heading"><strong>Expected body / 변수 예시 JSON</strong><span>{form.validateExact ? '일치 검증에 사용' : '변수 추출용으로만 사용'}</span></div><JsonArea value={form.expectedBody} onChange={value => set('expectedBody', value)} placeholder={'{\n  "id": 1\n}'} /><AssertionEditor assertions={form.assertions} variables={responseVariables} enabled={form.validateConditions} onAdd={addAssertion} onUpdate={updateAssertion} onConfirm={confirmAssertion} onSelectVariable={selectAssertionVariable} onRemove={removeAssertion} /></section>
