...
상단 `대시보드` 또는 프로젝트 사이드바의 `실행 대시보드`에서 웹 기능 테스트 실행 이력을 확인할 수 있습니다. `/dashboard`는 전체 프로젝트, `/dashboard?project=example-api`는 선택한 프로젝트를 표시하며 브라우저 새로고침과 뒤로 가기를 지원합니다.
...
조회 API는 `GET /api/dashboard?project=example-api.json&days=7&status=failed&page=1`입니다. `project`와 `status`는 생략할 수 있고 `status`는 `passed`, `failed`, `error`, `timeout`을 지원합니다. 응답에는 `summary`, `trend`, `items`, `total`, `page`, `pageSize`가 포함됩니다.
