...
상단 `대시보드` 또는 프로젝트 사이드바의 `실행 대시보드`에서 API 실행 현황을 확인합니다. `/dashboard`는 전체 프로젝트, `/dashboard?project=example-api`는 특정 프로젝트를 표시하며 새로고침과 뒤로 가기를 지원합니다.

- 최근 7일·30일·90일의 전체 실행 횟수, 성공률, 실패·오류·시간 초과 횟수, 평균 실행 소요 시간과 일별 추이를 제공합니다.
- 집계 단위는 웹의 `POST /api/run` 요청 한 번입니다. 여러 케이스나 파이프라인을 한 요청으로 실행하면 1회이며, 소요 시간에는 재시도와 파이프라인 전체 처리 시간이 포함됩니다. 개별 HTTP 응답 시간과는 다릅니다.
- 완료된 실행만 15초마다 갱신합니다. 이력은 결과별 필터와 20건 단위 페이지 이동을 지원합니다. 결과 필터는 목록에만 적용되며 상단 지표와 추이는 프로젝트·기간 기준을 유지합니다.
- 기간과 일별 추이는 UTC 날짜 기준이며, 이력의 실행 시각은 브라우저 현지 시간으로 표시합니다. 성공률은 성공 실행 / 전체 완료 실행이며 이력이 없으면 성공률·평균 시간은 `—`로 표시합니다.
- 도입 이후 웹에서 실행한 기록을 `data/execution-history.db`에 보관합니다. `STUDIO_DB_PATH`를 설정하면 해당 DB와 같은 디렉터리에 저장되므로 기존 Docker `data` 볼륨으로 보존됩니다. CLI 실행과 기존 로그의 과거 이력은 집계하지 않습니다.
- 이력에는 실행 대상·프로젝트·시각·소요 시간·종료 코드만 저장하며 요청/응답 본문과 인증 정보는 추가 저장하지 않습니다. 실행 상세 출력은 기존 실행 화면과 로그에서 확인합니다. 서버를 강제 종료해 완료되지 못한 실행은 기록되지 않습니다.

조회 API: `GET /api/dashboard?project=example-api.json&days=7&status=failed&page=1`. `project`와 `status` 생략 시 전체이며, `status`는 `passed`, `failed`, `error`, `timeout`을 지원합니다. 응답은 `summary`, `trend`, `items`, `total`, `page`, `pageSize`를 포함합니다. 종료 코드 0은 성공, 1은 실패, 기타 코드는 오류로 분류합니다. 여러 프로젝트가 포함된 실행은 각 프로젝트 조회에 포함되며 전체 조회에서는 한 번만 집계합니다.
